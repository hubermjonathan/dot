...
- Claude settings are split three ways: `settings.repo.json` (versioned, applies everywhere), `~/.claude/settings.work.json` (machine-local, work-specific, NOT versioned), and `~/.claude/settings.json` (the file Claude actually reads — produced by merging `machine ⊕ work ⊕ repo`, right wins). The statusline shows two indicators: 🆘 when repo keys are missing/different on the machine, 📥 when `settings.json` has leaves absent from both repo and work — the prompt to file each one into the appropriate layer.
