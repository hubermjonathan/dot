...
| `installer/` | `brew install` and `brew install --cask`, executes `post_link` and `provision` script lists. Entry points take `io.Writer` for combined stdout/stderr; the caller (typically `ui.Step`) owns buffering. Failures are reported as `*CommandError` |
